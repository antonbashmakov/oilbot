...
import {DeliveryOverview, OrderResolvedEvent, Stats, Payment, CustomerOverview, OrderCancelledEvent, Order, Comment, BroadcastTask} from "../../models";
...
adminApi.post("/orders/:id/consolidate", async (req: express.Request, res: express.Response) => {
  try {
    ...
    const event: OrderResolvedEvent = {
      ...
      idempotent_key: `order-resolved-${order.id}`,
      created_at: new Date(),
...
adminApi.post("/customers/:customerId/messages", async (req: express.Request, res: express.Response) => {
  try {
    ...
    const savedMessage = await messagingService.sendMessage(customer, text, thread_id);
...
adminApi.get("/customers/:customerId/messages", async (req: express.Request, res: express.Response) => {
  try {
    ...
    const customerMessages = await conversationMessageService.findByFilter({recipient: customerId, threadId: (threadId as string)});
